...
export default function Connect() {
  ...
  useEffect(() => {
    if (formData.name || formData.host || formData.password || formData.privateKey) return;
    if (savedConnections.length === 0) return;

    const sorted = [...savedConnections].sort(
      (a, b) => (b.lastConnected ?? 0) - (a.lastConnected ?? 0)
    );
    const pick = sorted[0];
    if (pick) handleSelectServer(pick);
  }, [formData.host, formData.name, formData.password, formData.privateKey, savedConnections]);
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-900 p-4">
      <Card className="w-full max-w-md shadow-lg">
        ...
        <form onSubmit={handleSubmit}>
            ...
            <CardFooter>
                <Button type="submit" className="w-full h-11 bg-transparent text-primary border border-primary hover:bg-primary hover:text-primary-foreground" disabled={loading}>
                    {loading ? (
                      <>
