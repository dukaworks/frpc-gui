...
FRPC GUI 允许您可视化地管理 FRPC (Fast Reverse Proxy Client) 配置文件。您无需再通过 SSH 登录服务器手动编辑 TOML/INI 文件，而是可以使用这个现代化的 Web 界面来添加、编辑、删除代理，管理多个服务器，并查看实时日志。
...
*   **多服务器支持**：保存并在多个 FRPC 服务器配置之间快速切换。
*   **实时日志**：查看正在运行的 FRPC 服务（Docker、Systemd 或进程）的实时日志。
*   **安全优先**：内置配置备份和“重启服务”时的安全检查机制。
